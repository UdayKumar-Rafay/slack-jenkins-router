...
In the Slack App dashboard → **Slash Commands**:
...
All three commands point to the same Lambda URL. The Lambda forwards the full Slack payload (which includes the command name) to Jenkins, and the router reads `SLACK_COMMAND` to distinguish between them.

**Bot permissions required** (`OAuth & Permissions` → `Bot Token Scopes`):
- `chat:write` — post messages
- `chat:write.public` — post to channels the bot hasn't been invited to (optional but convenient)
